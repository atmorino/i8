...
class FlowchartDrawer {
    ...
    /**
     * ノードの定義と順番を設定する
     * @param {string} definition - カンマ区切りのノード定義
     */
    setNodeDefinition(definition) {
        this.nodeDefinition = definition.split(',').map(node => node.trim());
    }
    ...
    parseInput(input) {
        ...
        // ノード情報を生成
        this.nodes = this.nodeDefinition.map((name, index) => ({
            name,
    ...
    /**
     * 接続が有効かどうかを判定する
     * @param {string} from - 開始ノード名
     * @param {string} to - 終了ノード名
     * @returns {boolean} 接続が有効な場合はtrue
     */
    isValidConnection(from, to) {
        const fromIndex = this.nodeDefinition.indexOf(from);
        const toIndex = this.nodeDefinition.indexOf(to);
    ...
    calculateNodeDistance(arrow) {
        const fromIndex = this.nodeDefinition.indexOf(arrow.from);
        const toIndex = this.nodeDefinition.indexOf(arrow.to);
     ...
     */
    doArrowsOverlap(a, b) {
        const aFrom = this.nodeDefinition.indexOf(a.from);
        const aTo = this.nodeDefinition.indexOf(a.to);
        const bFrom = this.nodeDefinition.indexOf(b.from);
        const bTo = this.nodeDefinition.indexOf(b.to);
